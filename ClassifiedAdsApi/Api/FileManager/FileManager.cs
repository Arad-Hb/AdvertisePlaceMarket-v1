...
public class FileManager(IWebHostEnvironment environment, IConfiguration configuration) : IFileManager
{
    ...
    public Task<FileManagerResult> SaveAdvertisementImageAsync(IFormFile file)
        => SaveImageAsync(
            file,
            "images/advertisements/originals",
            "images/advertisements/thumbnails",
    ...
    public Task<FileManagerResult> SaveHeroBannerAsync(IFormFile file)
        => SaveImageAsync(file, "images/hero-banners", null, createThumbnail: false);
        ...
        bool createThumbnail)
    {
        ...
        try
        {
            ...
            {
                var thumbnailPhysicalFolder = Path.Combine(
                    root,
                    thumbnailFolder.Replace('/', Path.DirectorySeparatorChar));
    ...
    private static void CreateThumbnail(
        ...
        string extension)
    {
        // WebP is accepted by the API, but GDI+ does not provide a built-in WebP encoder.
        // On non-Windows hosts System.Drawing is also not used. In those two cases we keep
        // a separate physical thumbnail path by copying the validated original image.
